...
## Segment plan (6 speakers, ~10 min)

| # | Speaker | Section | Time |
|---|---|---|---|
| 1 | Member A | Hook + problem + product pitch | 1:15 |
| 2 | Member B | Data & datasets + ER / data model | 1:30 |
| 3 | Member C | Relational DB: schema, constraints, triggers (live) | 1:45 |
| 4 | Member D | NoSQL + object storage: reviews, setlists, MinIO (live) | 1:30 |
| 5 | Member E | Application demo: customer journey (live) | 1:45 |
| 6 | Member F | Admin, security, performance + wrap-up | 1:45 |
...
- Datasets: a synthetic but realistic catalogue (21 users, 30 artists, 15 real
  venues, 50 concerts, 100+ ticket tiers, ~180 bookings, 220 follows),
...
- **Trigger 1 — seat inventory:** book a ticket in the UI; show
  ...
  booking → `AFTER UPDATE` trigger restores seats.
...
- Browse with **city + genre filters**; reload to show the Redis cache hit
  (`redis-cli MONITOR`).
- Book tickets with the **+/- stepper**; then try to exceed **6 per concert** →
  clear on-screen error, enforced by a DB `SUM(quantity)` check.
...
- Performance: show `benchmark.py` output — Redis cache ~20–30× faster than the
  uncached join; indexed vs full-scan; CPU/memory. Show the chart.
