...
6. Try to book more seats than are available → trigger raises, transaction
   rolls back, app shows the error.
7. Post a review → goes into `gigtrack.reviews` in Mongo.
8. Follow / unfollow an artist → M:N row inserted into `follows`.
...
| Task 7 — perf analysis (optional) | Use the Redis-cached vs uncached home page as the demo case |
